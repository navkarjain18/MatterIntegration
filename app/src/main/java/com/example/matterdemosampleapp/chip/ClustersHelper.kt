...
import com.example.matterdemosampleapp.CommissioningWindowStatus
...
class ClustersHelper @Inject constructor(private val chipClient: ChipClient) {
    ...
    suspend fun readApplicationBasicClusterAttributeList(
        ...
        return suspendCoroutine { continuation ->
            getApplicationBasicClusterForDevice(
                connectedDevicePtr, endpoint
            ).readAttributeListAttribute(object :
    ...
    suspend fun readBasicClusterVendorIDAttribute(deviceId: Long, endpoint: Int): Int? {
        ...
        return suspendCoroutine { continuation ->
            getBasicClusterForDevice(connectedDevicePtr, endpoint).readVendorIDAttribute(object :
    ...
    suspend fun toggleDeviceStateOnOffCluster(deviceId: Long, endpoint: Int) {
        ...
        return suspendCoroutine { continuation ->
            getOnOffClusterForDevice(connectedDevicePtr, endpoint).toggle(object :
    ...
    suspend fun setOnOffDeviceStateOnOffCluster(deviceId: Long, isOn: Boolean, endpoint: Int) {
        ...
        if (isOn) {
            // ON
            return suspendCoroutine { continuation ->
                getOnOffClusterForDevice(connectedDevicePtr, endpoint).on(object :
                    ChipClusters.DefaultClusterCallback {
                    override fun onSuccess() {
                    ...
                    override fun onError(ex: Exception) {
                        Log.e(
                            TAG, "Failure for setOnOffDeviceStateOnOffCluster -> ${ex.message}"
                        )
                        continuation.resumeWithException(ex)
                    }
                })
            ...
            // OFF
            return suspendCoroutine { continuation ->
                getOnOffClusterForDevice(connectedDevicePtr, endpoint).off(object :
                    ChipClusters.DefaultClusterCallback {
                    override fun onSuccess() {
                    ...
                    override fun onError(ex: Exception) {
                        Log.e(
                            TAG, "Failure for getOnOffDeviceStateOnOffCluster -> ${ex.message}"
                        )
                        continuation.resumeWithException(ex)
                    }
                })
    ...
    suspend fun getDeviceStateOnOffCluster(deviceId: Long, endpoint: Int): Boolean? {
        ...
        return suspendCoroutine { continuation ->
            getOnOffClusterForDevice(connectedDevicePtr, endpoint).readOnOffAttribute(object :
    ...
    suspend fun getMaxLevel(deviceId: Long, endpoint: Int): Int? {
        val connectedDevicePtr = try {
            chipClient.getConnectedDevicePointer(deviceId)
        } catch (e: IllegalStateException) {
            Log.e(TAG, "Can't get connectedDevicePointer.")
        ...
        return suspendCoroutine { continuation ->
            getLevelClusterForDevice(connectedDevicePtr, endpoint).readMaxLevelAttribute(object :
    ...
    suspend fun getMinLevel(deviceId: Long, endpoint: Int): Int? {
        val connectedDevicePtr = try {
            chipClient.getConnectedDevicePointer(deviceId)
        } catch (e: IllegalStateException) {
            Log.e(TAG, "Can't get connectedDevicePointer.")
        ...
        return suspendCoroutine { continuation ->
            getLevelClusterForDevice(connectedDevicePtr, endpoint).readMinLevelAttribute(object :
                ChipClusters.IntegerAttributeCallback {
                override fun onSuccess(value: Int) {
                    continuation.resume(value)
                }
    ...
    private fun getLevelClusterForDevice(
        devicePtr: Long, endpoint: Int
    ...
    suspend fun openCommissioningWindowAdministratorCommissioningCluster(
        ...
        val connectedDevicePtr = try {
            ...
            Log.e(TAG, "Can't get connectedDevicePointer. -> ${e.message}")
            return
        }
                ...
                object : ChipClusters.DefaultClusterCallback {
                    ...
                    override fun onError(ex: java.lang.Exception?) {
                        Log.e(
                            ...
                            "getAdministratorCommissioningClusterForDevice.openCommissioningWindow command failure -> ${ex?.message}"
                        )
                        continuation.resumeWithException(ex!!)
                    }
                },
            ...
            val callback = object : ChipClusters.IntegerAttributeCallback {
                override fun onSuccess(value: Int) {
                    when (value) {
                        ...
                        CommissioningWindowStatus.EnhancedWindowOpen.status, CommissioningWindowStatus.BasicWindowOpen.status -> {
