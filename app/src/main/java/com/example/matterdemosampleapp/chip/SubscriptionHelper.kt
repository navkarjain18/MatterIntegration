...
class SubscriptionHelper @Inject constructor(private val chipClient: ChipClient) {
    ...
    suspend fun awaitSubscribeToPeriodicUpdates(
        ...
        return suspendCoroutine { continuation ->
            Log.d(TAG, "subscribeToPeriodicUpdates()")
            ...
            Log.d(TAG, "attributePath: [${attributePath}]")
    ...
    suspend fun awaitUnsubscribeToPeriodicUpdates(connectedDevicePtr: Long) {
        Log.d(TAG, "awaitUnsubscribeToPeriodicUpdates()")
        return suspendCoroutine { continuation ->
    ...
    open class ReportCallbackForDevice(val deviceId: Long) : ReportCallback {
        ...
        override fun onError(
            ...
            if (attributePath != null) {
                Log.e(
                    ">>//",
                    "reportCallback: error on device [${deviceId}] for [${attributePath}] -> ${ex.message}"
                )
            }
            if (eventPath != null) {
                Log.e(
                    ">>//",
                    "reportCallback: error on device [${deviceId}] for [${eventPath}] -> ${ex.message}"
                )
            }
        }

        override fun onReport(nodeState: NodeState) {
            //Log.d( TAG ,"reportCallback: onReport")
            val debugString = nodeStateToDebugString(nodeState)
            //Log.d( TAG ,"------- BEGIN REPORT -----")
            //Log.d( TAG ,debugString)
            //Log.d( TAG ,"------- END REPORT -----")
        }

        override fun onDone() {
            Log.d(">>//", "reportCallback: onDone")
        }
    }
    ...
    open class SubscriptionEstablishedCallbackForDevice(val deviceId: Long) :
        ...
        override fun onSubscriptionEstablished(subscriptionId: Long) {
            Log.d(">>//", "onSubscriptionEstablished(): subscriptionId [${subscriptionId}]")
        }
    }
